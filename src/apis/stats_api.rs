...
/// struct for typed errors of method [`stats_slash_current`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum StatsSlashCurrentError {
...
pub async fn stats_slash_current(
    configuration: &configuration::Configuration,
    pretty: Option<bool>,
) -> Result<models::StatsCurrent200Response, Error<StatsSlashCurrentError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/stats/current", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_content = local_var_resp.text().await?;
        let local_var_entity: Option<StatsSlashCurrentError> =
