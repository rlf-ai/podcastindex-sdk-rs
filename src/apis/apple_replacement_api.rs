...
/// struct for typed errors of method [`lookup`]
...
pub async fn lookup(
    ...
    entity: &str,
    pretty: Option<bool>,
) -> Result<models::Search200Response, Error<LookupError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/lookup", local_var_configuration.base_path);
    let mut local_var_req_builder =
        local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());

    local_var_req_builder = local_var_req_builder.query(&[("id", &id.to_string())]);
    local_var_req_builder = local_var_req_builder.query(&[("entity", &entity.to_string())]);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_content = local_var_resp.text().await?;
        let local_var_entity: Option<LookupError> = serde_json::from_str(&local_var_content).ok();
