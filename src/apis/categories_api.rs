...
/// struct for typed errors of method [`categories_slash_list`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CategoriesSlashListError {
...
pub async fn categories_slash_list(
    configuration: &configuration::Configuration,
    pretty: Option<bool>,
) -> Result<models::CategoriesList200Response, Error<CategoriesSlashListError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/categories/list", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_content = local_var_resp.text().await?;
        let local_var_entity: Option<CategoriesSlashListError> =
